...
task runr, "Runs text_adventure for current platform":
 exec &"nim c -r {releaseOpts} -o:text_adventure src/main.nim"

task rund, "Runs debug text_adventure for current platform":
 exec &"nim c -r {debugOpts} -o:text_adventure src/main.nim"

task release, "Builds text_adventure for current platform":
 exec &"nim c {releaseOpts} -o:text_adventure src/main.nim"

task webd, "Builds debug text_adventure for web":
 exec &"nim c {debugOpts} -d:emscripten -o:text_adventure.html src/main.nim"

task webr, "Builds release text_adventure for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:text_adventure.html src/main.nim"

task debug, "Builds debug text_adventure for current platform":
 exec &"nim c {debugOpts} -o:text_adventure_debug src/main.nim"
